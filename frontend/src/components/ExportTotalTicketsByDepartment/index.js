...
const ExportTotalTicketsByDepartment = ({ reports, filters }) => {
  const [exportOption, setExportOption] = useState("");

  // Agrupa os tickets por departamento
  ...
  const data = Object.entries(groupedData).map(([department, total]) => ({
  ...
  const exportToPDF = () => {
    ...
    doc.text("Tickets por Departamento", 14, 10);
    ...
    autoTable(doc, {
      head: [["Departamento", "Total de Tickets"]],
      body: data.map((item) => [item.Departamento, item["Total de Tickets"]]),
      startY: appliedFilters ? 35 : 20,
    });

    doc.save("tickets_por_departamento.pdf");
  };
  ...
  const exportToExcel = () => {
    ...
    const worksheetData = [
      ["Tickets por Departamento"], // Título
      [appliedFilters || "Sem filtros aplicados"], // Filtros
      [], // Linha vazia para separar do conteúdo
      ["Departamento", "Total de Tickets"], // Cabeçalho
      ...data.map((item) => [item.Departamento, item["Total de Tickets"]]),
    ];

    const worksheet = XLSX.utils.aoa_to_sheet(worksheetData);

    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, worksheet, "Tickets por Departamento");
    XLSX.writeFile(wb, "tickets_por_departamento.xlsx");
  };
...
export default ExportTotalTicketsByDepartment;